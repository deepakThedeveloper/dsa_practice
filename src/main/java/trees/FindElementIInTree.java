...
public class FindElementIInTree {
    public static void main(String[] args) {
        Node1 root = getRootNode();
        boolean element = findElementInTree(root, 710);
    ...
    private static boolean findElementInTree(Node1 root, int k) {
        for(Node1 child : root.children){
            if(child.value == k) {
                return true;
            }
            boolean val = findElementInTree(child, k);
            if(val){
                return true;
            }
        }
