...
public class DistanceOfEveryNodeWithRoot {
    public static void main(String[] args) {
        Node1 root = getRootNode();
        printDistanceBetweenRootAndEveryNode(root, 0);
    }

    private static void printDistanceBetweenRootAndEveryNode(Node1 root, int count) {
        for(Node1 child : root.children){
            printDistanceBetweenRootAndEveryNode(child, count+1);
            System.out.println(child.value+"->"+(count+1));
        }
