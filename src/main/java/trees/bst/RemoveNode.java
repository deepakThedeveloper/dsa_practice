...
// there are two approaches to remove node.
//1. in case of removing intermitten node which has both left and right child then place intermitten node with left max
//and remove left max from its original parent
//2. in same above case, replace the intermitten node with right node of intermitten node and and attach left child of
//intermitten node to extreme left of new right node.
