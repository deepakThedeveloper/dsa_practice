...
public class RodCuttingPblm {
    public static void main(String[] args) {
        ...
        int maxPrice4 = rodCutTabulationApproach2(rodLength, price);
        System.out.println("tabulationApproach2:"+maxPrice4);
    }
    ...
    private static int rodCutTabulationApproach2(int length, int[] prices){
        int[] dp = new int[length+1];
        for(int i=1; i<=length; i++){
            int max = Integer.MIN_VALUE;
            for(int j=i; j>0;j--){
                int cut = i-j;
                max = Math.max(max, dp[cut] + prices[j-1]);
            }
            dp[i] = max;
        }
        return dp[length];
